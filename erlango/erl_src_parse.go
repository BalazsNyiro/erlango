...
func erlSrcTokens_rangeDetect__connectToChars(
	...
	for position, charNow := range chars {
		if verbose {
			fmt.Println("Token to char, charNow:", charNow)
		}

		if skip_chars_with_tokens && chars[position].TokenConnected() {
			continue
		} // modify only the unprocessed chars, without Tokens
		...
		if verbose {
			tokenInfo(position, chars, tokens, inTokenDetection_activeCharsFound, conditionMemoryTemporaryWorkspace)
		}

		if !canBeOneCharWideTokenDetection { // here we know that the wanted token leng is bigger than 1
			if nowOpened || nowEscaped {
				continue
			} // the opener cannot be the closer: ".." pairs for example
...
func commentConditionOpener(chars []ErlSrcChar, position int, memory *conditionMemory) bool {
	if chars[position].TokenConnected() {
		return false
...
func commentConditionCloser(chars []ErlSrcChar, position int, memory *conditionMemory) bool {
	...
	if position == lastPos {
		return true
	} // this is the last char, we won't find a newline.
...
func generalConditionOpenerCharInPattern(chars []ErlSrcChar, position int, memory *conditionMemory, pattern string) bool {
	...
	if position > lastPos {
		return false
	} // can't look over the end of the chars
	if chars[position].TokenConnected() {
		return false
	}
...
func generalConditionCloser(chars []ErlSrcChar, position int, validPossibleBodyPattern string) bool {
	...
	if nextPos <= lastPos {
		if chars[nextPos].TokenConnected() {
			return true
		}
...
func multi_token_detect(chars []ErlSrcChar, verbose bool, detector func([]*ErlSrcToken, int, bool)) []ErlSrcChar {
	...
	for tokenId, _ := range tokenPtrs {
		detector(tokenPtrs, tokenId, verbose)
	}
